...
async function tasklistPids(filter: string): Promise<number[]> {
  if (!isWin) return [];
...
async function main(): Promise<void> {
  ...
  for (const p of await tasklistPids("IMAGENAME eq pnpm.cmd")) candidates.add(p);
  for (const p of await tasklistPids("WINDOWTITLE eq preview*")) candidates.add(p);
  for (const p of await portPids([webPort, apiPort])) candidates.add(p);
  for (const p of await pgrepPids(["vite preview", "dist/index.js"])) candidates.add(p);
