...
**[mới 2026-09-07] Stop kill toàn bộ process tree web + server** — nhánh `feat/tray-spawn`. Trước đó `pnpm start:stop` chỉ match `pnpm.cmd` qua `tasklist /FI "IMAGENAME eq pnpm.cmd"`, nhưng `cmd.exe /c start "" /B pnpm ...` tạo cây `cmd.exe → pnpm.mjs → cmd.exe → vite.js` — không có process nào tên `pnpm.cmd`. Fix: dùng `Get-CimInstance Win32_Process` qua file PS1 tạm (PS 5.1 escape `$_` hỏng khi gọi `-Command` trực tiếp, phải qua `-File`) để lấy `(pid, name, commandline)` cho `node.exe`/`pnpm.cmd`/`cmd.exe`, match regex: `apps\web\node_modules\.bin\..\vite\bin\vite.js`, `apps\server\dist\index.js`, `pnpm.mjs" exec vite preview`, `pnpm.mjs" exec node dist/index.js`. Vẫn giữ `netstat -ano` match port listener làm fallback. `taskkill /T /F` chain kill cả cây. E2E: start:hidden → port 3000 + 4173 listen → start:stop kill 3 PID → cả 2 port tắt.
