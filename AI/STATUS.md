...
**[mới 2026-09-07] feat(extension) — clip editorial (solution article)** — extension giờ clip được cả tab Editorial của LeetCode. Parser mới `apps/extension/src/parsers/editorial.ts`: tìm container `[class*="solution-markdown_markdown"]` (verify DOM thật trên leetcode.com/problems/two-sum/editorial), clean HTML giữ iframe video vimeo + playground code (bỏ svg/anchor icon), guard marker (Solution Article/Approach/vimeo/playground) để trang /description không bị nhầm. `ProblemClip.editorial` (shared + extension local type), DB migration `0003_add_editorial` (cột `problems.editorial`), server `importSchema` + `importClip`/`updateClip`/`hydrate`/`getById` truyền editorial qua engine + DB. content.js bundle lại. 58 tests extension pass (5 mới), `pnpm -r build` + `pnpm -r test` pass, migration verified trên DB thật (11 rows). Commit `7c2185c`.

**[mới 2026-09-07] feat/docs-db — docs data vào SQLite thay JSON bundle** — bảng `doc_files` + `doc_sections` (migration 0002), seeder `pnpm --filter=@leetcode/database db:seed-docs` (idempotent, từ JSON artifact). `javascript-docs` bỏ 28 static JSON import — data nạp qua `setDocsData()` (server hydrate từ DB, tự seed khi rỗng; test nạp fixture; web fetch API). Server `/api/docs/{search,categories,section/:id,meta}` (Zod 400/404 chuẩn). Web Knowledge + CodeEditor suggest fetch qua API — **bundle JS web ~6.5MB → 1.49MB**. Docker runtime copy JSON data cho auto-seed volume trống. Gotcha: keyword "constructor" đụng prototype — `buildKeywordIndex` dùng null-prototype. `pnpm -r build` + `test` pass (186 tests). Xem `AI/history/2026-09/javascript-docs-db.md`.

**[mới 2026-09-07] refactor(javascript-docs) — xóa tmp_reference, generate.py đọc src/docs** — `tmp_reference/` + `tmp_reference_vi/` (nguồn .md clone từ Kernix13/javascript-cheat-sheet) đã xóa sau khi chuẩn hóa vào `src/docs/en|vi`; `generate.py` trỏ `DEFAULT_SRC`/`DEFAULT_SRC_VI` vào `src/docs/{en,vi}`, JSON output chỉ lệch thứ tự tags. JSON `src/data/` giờ là artifact trung gian cho seeder (docs-db). Commit `540cccf`.
