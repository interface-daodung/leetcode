...
2. Gọi backlog đó — thực hiện quy trình sửa bug cổ điển trên **duy nhất backlog đang chọn**:
   ...
   7. Test regression (Vitest `vitest run` hoặc test thủ công liên quan).
    8. Cập nhật documentation nếu cần (ghi `AI/context/decisions.md` hoặc `AI/history/` — kiến trúc/codebase dùng graphify `graphify-out/graph.json`, `AI/index/`, `AI/ARCHITECTURE.md` đã archive).
